...
namespace Coyote.DataFrame
{
    ...
    /// <para>Next, another byte representing an unsigned integer (0-255) is read in and used to indicate the type of the field. If it is a numeric or other fixed type, the appropriate number of bytes are read in. If a variable type is indicated then the next S32 integer (4-bytes) is read as the length of the data. S32 is used to support nesting of frames within frames which can quickly exceed U16 values of 32767 bytes in length.</para>
    ...
    public class DataField : ICloneable
    {
        ...
        /// <summary>
        /// Determine if this field is undefined
        /// </summary>
        /// <remarks>
        ...
        protected internal DataField(short type, byte[] value)
        {
            this.type = type;
            this.value = value;
        }
        ...
        public DataField(BinaryReader br)
        {
            ...
            if (nameLength > 0)
            {
                ...
                br.Read((byte[])nameData, 0, nameData.Length);
            ...
            if (datatype.Size < 0)
            {
                ...
                byte[] u16 = br.ReadBytes(2);

                // ...and convert it to a length using big-endian encoding
                uint length = ByteUtil.RetrieveUnsignedInt(u16, 0);
        ...
        public static short GetType(object obj)
        {
        ...
        public static byte[] Encode(object obj, short type)
        {
        ...
        private object GetObjectValue(short typ, byte[] val)
        {
        ...
        public virtual void Write(BinaryWriter bw)
        {
            ...
            if (@value != null)
            {
                ...
                if (datatype.Size < 0)
                {
                    // write the length as a S32 bit value
                    bw.Write(ByteUtil.RenderInt(@value.Length));
                }
        ...
        private static string GetTypeName(short code)
        {
        ...
        protected internal static int GetTypeSize(short code)
        {
        ...
        protected internal static FieldType GetDataType(short typ)
        {
        ...
        private string GetStringValue(short typ, byte[] val)
        {
            FieldType datatype = GetDataType(typ);
